...
> 解决重复误报磨损信任的问题：开发者标记过的误报不应再次出现在报表中。

### 抑制方式

在 `.hermes/review-config.json` 中添加 `suppressions` 字段：
    ...
    "suppressions": [
      {
        ...
        "line": 45,
        "rule": "P9-AI-ux_gap",
        "reason": "已核实——支付失败跳转订单详情而非原地展示错误，是产品设计决策",
        "suppressed_at": "2026-07-27"
      }
    ]
  }
}
...
| `file` | ✅ | 相对于项目根目录的文件路径 |
| `line` | ✅ | 行号（`0` = 整文件抑制） |
| `rule` | ✅ | 规则 ID（格式 `{阶段}-{来源}-{类别}`，如 `P2.1-hardcoded_secret`、`P9-AI-ux_gap`） |
| `reason` | ✅ | 抑制原因（**强制填写**，防止无脑抑制） |
| `suppressed_at` | — | 时间戳，缺则 P10 报表统计行提示"缺少时间戳" |

### 抑制行为

- P10 报表生成时，缺陷在进入统计前先过抑制表
- 命中抑制的缺陷 → 归入报表底部 `📎 已抑制告警` 折叠区（显示原因，不计入缺陷统计）
- 代码变更导致行号偏移 → 抑制**自动失效**，下轮审查重新评估
- `line: 0` 整文件抑制 → 仅对该文件生效，文件被重命名或删除后失效

### 与 🤖 标签的关系

- 抑制是**开发者主动操作**："我确认这不是问题"
- 🤖 标签是**系统诚实声明**："我是猜的，你需要核实"
- 配合形成闭环：🤖 提醒 → 开发者核实 → 确认误报 → 抑制 → 下次不再出现
...
⏭️ 无
📦 总览矩阵（含 `~` 前缀注释）· 双板块拆分（技术故障/用户投诉风险）· 状态列（✅完整/⚠️部分/⏭️跳过）· 排序规则 · CI兜底提示 · 跳过项统计 · 📎已抑制告警（折叠区）· AI建议附录
...
11. **抑制需写原因**：`reason` 字段强制填写，防止开发者无脑抑制后遗忘原因
12. **抑制失效**：代码变更导致行号偏移时抑制自动失效，避免"改了代码还在抑制旧行号"
